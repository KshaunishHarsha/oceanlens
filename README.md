...
> foundation, linked analysis state, and a real-data layer). The operational
> UI is Phase 3+.

## Requirements

- Node ≥ 20 (developed on Node 26)
- No backend, login, API key, or network access at run time.
...
The app reads a **locally cached extract of real public data** from
`public/data/real/` (committed, ~13 MB / ~5 MB gzipped). Nothing is fetched from
the network while the app runs.
...
`.cache/raw/` (raw downloads) is git-ignored; `public/data/real/` (the normalised
cache the app reads) is committed. HYCOM's NCSS service is slow and intermittent —
the prepare script retries with backoff.
...
See [`docs/architecture.md`](docs/architecture.md). In brief: Vite + React +
TypeScript (strict) + Zustand; one `OceanDataAdapter` boundary with a cached
real-data implementation and four future-source placeholders; a single linked
analysis store; hand-rolled SVG for charts; a canvas-2D + SVG oblique scene
(Phase 4) behind a renderer interface.
