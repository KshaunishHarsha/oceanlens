...
| Repo state | Backend: **101 pytest**, untouched by every entry below (a `routes_briefing.py`/`briefing_service.py` + `EvidencePanel/BriefingPanel.tsx` feature was added to the tree by work outside this document's own sessions — present and passing; now documented below under "Briefing Mode"). `SceneStage` renders a real Three.js/WebGL scene with clickable real Argo markers, a real vendored coastline, and a fixed depth-box frame the slice plane visibly moves inside as depth changes; recovers correctly from a lost WebGL context; verified working at both the desktop (1600x1000) and 300px-control-rail (1440x900) breakpoints. `EvidencePanel`'s three tabs are all real and verified legible at both breakpoints: Profile, Comparison, Provenance. `/model-column` genuinely snaps to the nearest real timestamp; `/collocation` returns an honest `422` for `currentSpeed`/`chlorophyll`. CommandBar's search is now real (find any loaded Argo observation by id/name/WMO). **238 frontend tests green** (22 skipped without a live backend), tsc clean, build OK (85 modules). |

## Briefing Mode — grounded AI explanation layer (documented here, code predates this doc entry)

**A third `WorkspaceMode` (`'workspace' | 'briefing' | 'outreach'`, `analysisStore.ts`),
toggled by the "Briefing mode" button in `CommandBar`.** When active, `EvidencePanel`
swaps its three-tab body for `BriefingPanel.tsx` — a plain-language explanation of the
**currently selected observation's already-computed collocation evidence**, generated by
a real OpenAI Responses API call on the backend. It is explicitly NOT a general chat
assistant, cannot browse or fetch anything itself, and cannot alter or invent a scientific
result.

- **Backend**: `POST /api/v1/briefing/{observation_id}` (`routes_briefing.py`) takes an
  optional `question` string, computes the same real `compute_collocation()` evidence the
  Comparison tab already uses (via `collocation_service.get_collocation`), and passes
  **only that evidence** — observation/model source, timestamps, distance, time offset,
  sample count, RMSE, mean bias, interpretation sentence, per-depth-band verdicts, plus an
  explicit "historical, not live, not a forecast" label — to `briefing_service.py`'s
  `generate_briefing()`. That function calls `https://api.openai.com/v1/responses` with a
  locked system prompt (`_INSTRUCTIONS`) that forbids adding any measurement, cause,
  hazard, recommendation, source, or certainty not present in the evidence payload, and
  forbids ever calling the data real-time. The model receives no tools and no other data
  access — it can only rephrase/reason over the JSON evidence blob it was handed.
- **Same honesty gates as everywhere else**: reuses `UnsupportedCollocationVariableError`
  (422 for `currentSpeed`/`chlorophyll` — no observed-side data to ground a briefing in),
  `ObservationNotFoundError`/`NoModelColumnError` (404), and a 502 if OpenAI returns no
  usable text. If `OCEANLENS_OPENAI_API_KEY` (or `OPENAI_API_KEY`) isn't set on the
  backend, the route returns a `503` with an explicit "Briefing mode is not configured"
  detail rather than silently failing or fabricating an answer. Model is `gpt-5`
  (`settings.openai_model`, backend `app/config.py`), configurable via env var.
- **Frontend** (`BriefingPanel.tsx`): gated to a real selected observation
  (`EmptyState` otherwise) and to temperature/salinity (`UnavailableNote` otherwise — the
  same variable gate the Profile/Comparison tabs already use, since a briefing can only be
  grounded in evidence that exists). Three canned quick-ask chips ("What does the
  model-observation agreement show?", "What should I be cautious about in this evidence?",
  "Summarise this selected observation in plain language.") plus a free-text question box
  (500-char cap, sent verbatim). Renders the real answer text plus `generated_by` ("OpenAI,
  constrained to the selected cached evidence") and the real historical-window label —
  never presented as if it were a live assistant or a new data source.
- **Not independently re-verified by this document's own sessions** — behaviour above is
  read directly from the current source (`backend/app/services/briefing_service.py`,
  `backend/app/api/routes_briefing.py`, `src/ui/EvidencePanel/BriefingPanel.tsx`,
  `src/ui/EvidencePanel/EvidencePanel.tsx`, `src/ui/CommandBar.tsx`,
  `src/state/analysisStore.ts`) as of 2026-09-19, not re-tested end-to-end against a live
  OpenAI key in this session. Demo risk: it requires a configured API key and outbound
  network access to `api.openai.com` at presentation time — confirm both work before
  relying on it live, since the honest failure mode (503/502) is visible to the audience
